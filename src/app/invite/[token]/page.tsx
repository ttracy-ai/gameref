...
export default function InvitePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/invite/${token}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.error) setState({ status: "invalid" });
        else setState({ status: "ready", projectName: d.projectName });
      })
  ...
  useEffect(() => {
    ...
    fetch(`/api/invite/${token}`, { method: "POST" })
      ...
      .then((d) => {
        if (d.projectId) {
          setState({ status: "done", projectName: d.projectName });
      ...
      .catch(() => setState({ status: "invalid" }));
  }, [state.status, sessionStatus, token, router]);

  const projectName =
    (state.status === "ready" || state.status === "done" || state.status === "joining")
      ? state.projectName
      : null;
        ...
        >
          ...
          <div className="flex flex-col items-center mb-8">
            <div
              ...
              style={{
                ...
                boxShadow: "0 0 20px rgba(132,204,22,0.12)",
              }}
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={LOGO} alt="Plan A Project" className="w-12 h-12 object-contain" />
            </div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-100">Plan A Project</h1>
            <p className="mt-0.5 text-xs font-semibold tracking-[0.2em] uppercase" style={{ color: "rgba(132,204,22,0.65)" }}>
              planaproject.io
            </p>
          </div>
          ...
          {state.status === "loading" && (
            <div className="flex flex-col items-center gap-3 py-4">
              ...
              <p className="text-sm text-neutral-500">Validating invite…</p>
            </div>
          )}

          {state.status === "invalid" && (
            <div className="text-center py-2">
              <p className="text-sm text-red-400 font-medium">This invite link is invalid or has been revoked.</p>
              <p className="mt-1 text-xs text-neutral-600">Ask the project owner to generate a new one.</p>
            </div>
          )}

          {(state.status === "ready" || state.status === "joining") && sessionStatus === "unauthenticated" && (
            <div className="flex flex-col gap-3">
              <div className="text-center mb-1">
                <p className="text-sm text-neutral-300">You&apos;ve been invited to join</p>
                <p className="text-base font-semibold text-slate-100 mt-0.5">{projectName}</p>
                <p className="text-xs text-neutral-500 mt-1">Sign in to accept the invitation.</p>
              </div>
              <div className="flex items-center gap-3 my-1">
                <div className="flex-1 h-px" style={{ background: "rgba(255,255,255,0.05)" }} />
                <span className="text-[10px] text-slate-700 uppercase tracking-[0.15em]">sign in with</span>
                <div className="flex-1 h-px" style={{ background: "rgba(255,255,255,0.05)" }} />
              </div>
              <button
              ...
              <button
                onClick={() => signIn("discord", { callbackUrl: `/invite/${token}` })}
                className="w-full flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-slate-400 bg-white/[0.03] border border-white/[0.07] hover:bg-lime-500/[0.07] hover:border-lime-500/20 hover:text-slate-200 transition-all duration-200"
              >
          ...
          {(state.status === "joining" && sessionStatus === "authenticated") && (
            <div className="flex flex-col items-center gap-3 py-4">
              <Loader2 size={20} className="animate-spin" style={{ color: "rgba(132,204,22,0.7)" }} />
              <p className="text-sm text-neutral-400">Joining <span className="text-slate-200">{projectName}</span>…</p>
            </div>
          )}

          {state.status === "done" && (
            <div className="flex flex-col items-center gap-2 py-4 text-center">
              <div className="w-8 h-8 rounded-full flex items-center justify-center mb-1" style={{ background: "rgba(132,204,22,0.15)", border: "1px solid rgba(132,204,22,0.3)" }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(132,204,22,0.9)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
              </div>
              <p className="text-sm font-medium text-slate-200">You&apos;ve joined <span className="text-slate-100">{state.projectName}</span></p>
              <p className="text-xs text-neutral-500">Redirecting you to the app…</p>
            </div>
          )}
